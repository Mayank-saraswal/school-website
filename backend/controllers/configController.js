...
// @route   GET /api/config/subjects/:class
...
export const getSubjectsForClass = async (req, res) => {
  try {
    ...
    // Get subjects for the class and medium
    const subjects = await Subject.getSubjectsForClass(className, teacherMedium);

    if (!subjects || subjects.length === 0) {
      return res.status(404).json({
        success: false,
        message: `No subjects found for ${className} (${teacherMedium} Medium)`
      });
    }
...
// Admin Subject Management Functions
...
export const createOrUpdateSubjects = async (req, res) => {
  try {
    const { className, medium, academicYear, subjects } = req.body;
    ...
    const validatedSubjects = subjects.map((subject, index) => {
      if (typeof subject === 'string') {
        return {
          name: subject.trim(),
          code: subject.trim().substring(0, 3).toUpperCase() + (index + 1),
          isOptional: false,
          maxMarks: 100
        };
        ...
        return {
          ...
          code: subject.code?.trim().toUpperCase() || subject.name?.trim().substring(0, 3).toUpperCase() + (index + 1),
    ...
    console.log('Creating subjects with data:', {
      class: className, // Note: using 'class' for schema
      medium: medium,
      academicYear: year,
      subjects: validatedSubjects
    });

    // Check if subjects already exist for this class-medium-year combination
    let subjectDocument = await Subject.findOne({
      class: className, // Fixed: using 'class' field name
      medium: medium,
      academicYear: year
    });
    ...
    if (subjectDocument) {
      // Update existing
      subjectDocument.subjects = validatedSubjects;
      subjectDocument.lastUpdatedBy = req.admin?.name || 'Admin';
      await subjectDocument.save();
      console.log('Updated existing subject document:', subjectDocument._id);
    } else {
      // Create new
      subjectDocument = new Subject({
        class: className, // Fixed: using 'class' field name
        medium: medium,
        academicYear: year,
        subjects: validatedSubjects,
      ...
      await subjectDocument.save();
      console.log('Created new subject document:', subjectDocument._id);
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        academicYear: year,
        subjects: validatedSubjects,
        totalSubjects: validatedSubjects.length
      }
    });
    ...
    if (error.code === 11000) {
      return res.status(400).json({
        success: false,
        message: 'इस कक्षा और माध्यम के लिए विषय पहले से मौजूद हैं / Subjects already exist for this class and medium'
      });
    }

    res.status(500).json({
      success: false,
      message: 'विषय बनाने/अपडेट करने में त्रुटि / Error creating/updating subjects',
      error: process.env.NODE_ENV === 'development' ? error.message : 'Internal server error'
    });
  }
};
...
export const getAdminSubjects = async (req, res) => {
  try {
    const { className, medium, academicYear } = req.query;
...
// @desc    Delete subjects for a specific class-medium-year
...
export const deleteSubjects = async (req, res) => {
  try {
    ...
    const subjectDocument = await Subject.findById(id);
    if (!subjectDocument) {
      return res.status(404).json({
        success: false,
        message: 'विषय दस्तावेज़ नहीं मिला / Subject document not found'
      });
    }

    // Soft delete by setting isActive to false
    subjectDocument.isActive = false;
    subjectDocument.lastUpdatedBy = req.admin?.name || 'Admin';
    await subjectDocument.save();

    res.status(200).json({
      success: true,
      message: `${subjectDocument.class} (${subjectDocument.medium}) के विषय सफलतापूर्वक हटाए गए / Subjects for ${subjectDocument.class} (${subjectDocument.medium}) deleted successfully`
    });
...
export const getSubjectOptions = async (req, res) => {
  try {
    ...
    const mediums = await Subject.distinct('medium', { isActive: true });
    const years = await Subject.distinct('academicYear', { isActive: true });
