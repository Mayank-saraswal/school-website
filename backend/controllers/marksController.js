...
// @desc    Get subjects for teacher's class and medium
...
export const getSubjectsForTeacher = async (req, res) => {
  try {
    ...
    // Get teacher info for default class/medium if not provided
    const teacherId = req.teacher.id;
    const teacher = await Teacher.findById(teacherId);
    ...
    // Use provided class/medium or fallback to teacher's assigned class/medium
    const targetClass = className || teacher.classTeacherOf;
    const targetMedium = medium || teacher.medium;

    if (!targetClass || !targetMedium) {
      return res.status(400).json({
        success: false,
    ...
    // Get subjects for the specified class and medium
    const subjects = await Subject.getSubjectsForClass(targetClass, targetMedium);

    if (!subjects || subjects.length === 0) {
      return res.status(404).json({
        success: false,
        message: 'Subjects not configured by Admin for this class and medium'
      });
    }

    // Return just the subject names as specified in requirements
    ...
    console.error('Get subjects for teacher error:', error);
    res.status(500).json({
      success: false,
